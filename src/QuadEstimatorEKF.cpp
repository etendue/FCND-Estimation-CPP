...
void QuadEstimatorEKF::UpdateFromIMU(V3F accel, V3F gyro)
{
  ...
  //apply 2) using equation from 7.1.2
  Quaternion<float> qt;
  Quaternion<float> dq;
  qt = Quaternion<float>::FromEuler123_RPY(rollEst, pitchEst, ekfState(6));
  dq = dq.IntegrateBodyRate(gyro,dtIMU);
  Quaternion<float> qt_Est = dq * qt;
  float predictedRoll = qt_Est.Roll();
  float predictedPitch = qt_Est.Pitch();
  ekfState(6) = qt_Est.Yaw();
